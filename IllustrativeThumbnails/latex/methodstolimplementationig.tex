...
%	So the use of such advanced methods like Poisson equation is pointless, since the offer, providing natural edges and smooth transition, matches not even the input screenshot. 
	...
	Although reading the title of the thumbnails may be slower than the software recognition through low-level visual data, this aspect needs to !take count!? occur? Nem vagyok benne biztos hogy értem a logikát ebben a mondatban for the better visualization of the actual content area.%TODO{trade off} 
	...
	This way not only the silhouette but also the whole body of the letters and the space between them is marked salient. ! itt seams volt, csak ellenőrizd hogy mark-kal is jót jelent-e a mondat
	...
	For the search of UI widget three heuristic were developed.
	...
	Finally, the UI area is bounded by a straight line, !while! and? the horizontal one is longer than the vertical.
	Thus, the middle of the screen is taken as reference data for the investigation and is not checked for !the unimportant! sztem ez nem kell, ezt az előbb írtad le h ezek nem fontosak UI elements.
	...
	!Because of the use of color histograms and the assumption that the UI has similar background color, which is different from the content, it is important to investigate a coherent data, so the slices should cover only UI or only content area, and not a mix of them. Najó ezen még gondolkodok
	...
	But this pixel row is not necessarily an actual line or edge of the image, it is only a horizontal path, where every member has the same color. ! Ezt sztem jó h beleírtad
	...
	Therefore, the task is to find a horizontal pixel path, which has already passed the widget location area, but still belongs to the UI area, i.e. it contains the background color or the line at the edge of the UI and content area.\par
	\begin{figure}[h]
		\centering		
	...
	To sum up, to identify a string as a word, it is not sufficient to pass the first check, it  also need  to have a corresponding special histogram !to be labeled as word! ezt kitörölheted, úgy kezded a mondatot, hogy to identify a string as a word :P. 
	Figure~\ref{fig:strings} shows the output of the text detection algorithm. 
	The light blue color labels the possible word regions, thus in the result of the connected? :D ez tényleg connected? component analysis the darker color denotes the actual words found by the algorithm.\par 
	\begin{figure}
		\centering
		\begin{subfigure}[b]{0.45\columnwidth}
			\centering
	...
	A path map is thus generated, where all past possibilities belonging to the previous start pixels are stored, and where every new try that occurs when examining the next start pixel can be used as a look up! too inform., esetleg reference?.
	...
	To calculate the costs of a possible switch between the columns the importance value of the neighboring pixels are weighted by $\sqrt{5}$, $\sqrt{2}$ and !one! ? another?  according to their distance.
	...
	The next point is displayed in a/the blue square.
	...
	In this case the seam carving algorithm continues only for the remaining parameter, thus it is only looking for either horizontal or vertical seams, !since the width or the height already hit the preconditions, until it reaches one of the conditions above! Szerintem ez egyértelmű, már leírtad az előző mondatban. 
