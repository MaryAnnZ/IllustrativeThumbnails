...
All before mentioned approaches work with some kind of feature combining algorithms.
After choosing the interesting areas, which need to stay recognizable also on the resulting image, they will be merged into one output picture according to their methods.
...
A modified Poission equation using a guidance field needs to be solved for every color in the color space, where the guidance field is calculated from the gradients of the input image.
...
Combining and accumulating the color data promises very smooth results, naturally appearing joining areas.
...
So the use of such advanced methods like Poisson equation is pointless, since the offer, providing natural edges and smooth transition, matches not even the input screenshot. 
	...
	In the case of thumbnails there are other ways to get information about what application are actually opened, for example giving a title containing the running application's name.
	...
	The content matters and not the surface of these applications.
	...
	In this section these three main sections are discussed, and an overview about the algorithm is shown.
