...
So the use of such advanced methods like Poisson equation is pointless, since the offer, providing natural edges and smooth transition, matches not even the input screenshot. 
	...
	Then the application is compared to Adobe Photoshop Forras, their advantages and disadvantages are described.
	...
	Foto google, pdf \par 
	Since the histograms of the border area, the central area and the slices between them are sequentially compared, the value of their correlation is crucial by assigning them to one or to the other part of the image.
	...
	If this parameter is small (50) every bright area can be classified as text, even if the region is only neighboring to a word.
	...
	The set of possible words need to be sorted first, if they actually can be words according to their size.
	...
	For experimental investigations the text areas were set for not salient, so the images and not the words is saved. 
	 When the thumbnail needs to get really small, it  is better to save the image data instead of the text,since it would be impossible, that the remaining text is still  readable. 
	...
	When the threshold is set hovewer for a bigger value, the application switches at the very end of the process.
	In this case the the application works noticeably slower, and additionally in causes more artifacts then usual resampling.
	foto diablo 
	...
	Comparing the images to the outputs of the thumbnail algorithm, it is striking how much smaller and less readable the words are.
	...
	So screenshots with bigger and clearly defined UI areas have shorter run time, whereas inputs like gallery applications and computer games take longer. 
	
	\chapter{Future Work}
	This application according to the chapter before is useful for creating illustrative thumbnails, but there are certain implementations to extend its scope. 
	There are two main areas, where certain improvements can be applied: performance and the software methodology.
	Although the current application provides reasonable results, described in the chapter before, there are several approaches, which make the present method even robuster and easier to use.
	...
	In addition, the CUDA system can be simply integrated into the current application, since it is developed for the programming languages C, C++ and Fortran, and C++ is used also in this project.   
	...
	The in the current project implemented methods  are able to handle the common cases, but it can became more robust and usable in even more special cases with some improvement.\par 
	There are several approaches, which besides the low level pixel data examination, tries to find also semantical objects on the input.
	...
	The perceptual seam carving algorithm Forras based on the Human Attention Model calculates not only the color changes but also the occurance of the facial information.
	...
	In case of seam carving it is not the goal to recreate the eliminated area, but with this algorithm it is possible to make the transition smoother.
